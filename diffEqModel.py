...
def createLongData(data, diag, scanTimepts, partCode, ageAtScan):
  ...
  for p in range(nrParticipants):
    ...
    # print('longDiagAllTmpts', longDiagAllTmpts)
    # print('longAgeAtScan', longAgeAtScan)
    # print(asda)

  return longData, longDiagAllTmpts, np.array(longDiag), longScanTimepts, longPartCode, longAgeAtScan

def calcDiffData(longData, longAgeAtScan, longDiag, longPartCode):
  """
   function filters all NaNs automatically, but returns arrays of different dimensions for every biomarker
  ...
  # dXdTdata = np.nan * np.ones((nrParticipants, nrBiomk), float)
  # avgXdata = np.nan * np.ones((nrParticipants, nrBiomk), float)
  dXdTdata = np.empty(nrBiomk, dtype=object)
  avgXdata = np.empty(nrBiomk, dtype=object)
  filtDiagB = np.empty(nrBiomk, dtype=object)
  for b in range(nrBiomk):
    dXdTdata[b] = np.nan * np.ones(nrParticipants)
    avgXdata[b] = np.nan * np.ones(nrParticipants)
  ...
  for p in range(nrParticipants):
    ...
    for b in range(nrBiomk):
      ...
      fitIndices = np.logical_not(np.logical_or(np.isnan(longData[p][:,b]), np.isnan(longAgeAtScan[p])))

      if np.sum(fitIndices) >= 2:
        polyCoeff = np.polyfit(longAgeAtScan[p][fitIndices], longData[p][fitIndices,b], deg=1) # shape(P, K),
        # print('fitIndices', fitIndices)
        # print('longData[p][fitIndices,b]', longData[p][fitIndices,b])
        # print('longAgeAtScan[p][fitIndices]', longAgeAtScan[p][fitIndices])
        print('polyCoeff', polyCoeff)
        # highest power first
        ...
        dXdTdata[b][p] = polyCoeff[0]
        avgXdata[b][p] = np.nanmean(longData[p][:,b], axis=0)
  ...
  print('estimNoise', estimNoise)
  for b in range(nrBiomk):
    nnMask = np.logical_not(np.isnan(dXdTdata[b]))
    print('---------b', b)
    print('dXdTdata.shape', dXdTdata[b].shape)
    dXdTdata[b] = dXdTdata[b][nnMask]
    avgXdata[b] = avgXdata[b][nnMask]
    print('dXdTdata[b].shape', dXdTdata[b].shape)
    filtDiagB[b] = longDiag[nnMask]
    print(np.logical_not(nnMask).shape)
    print('longPartCode[np.logical_not(nnMask)]', [longPartCode[i] for i in np.where(np.logical_not(nnMask))[0]])

  # print('dXdTdata', dXdTdata)
  # print('avgXdata', avgXdata)
  # print('estimNoise', estimNoise)
  # print(adsa)

  for b in range(nrBiomk):
    assert not np.isnan(avgXdata[b]).any()
    assert not np.isnan(dXdTdata[b]).any()
  assert not np.isnan(estimNoise).any()
  # print(adsa)

  return dXdTdata, avgXdata, estimNoise, filtDiagB
  ...
  if np.any(dXdTpred < 0) and np.any(dXdTpred > 0):
    ...
    for i in range(1,nrPoints):
      ...
      if newZeroCrossFlag != oldZeroCrossFlag: # then add the previous section
        ...
        # changed on 12Feb2019 due to DEM not working on BM-confirmed subjects in AD
        isWithinRange = (xPred[xIndMin] < overlapRange[1] and overlapRange[0] < xPred[i-1])
        # isWithinRange = not (xPred[xIndMin] > overlapRange[1] or overlapRange[0] > xPred[i - 1])
    ...
    # isWithinRange = not (xPred[xIndMin] > overlapRange[1] or overlapRange[0] > xPred[nrPoints-1])
    isApproved.append(isEnforcedDir and isWithinRange)
    isEnforcedDirList.append(isEnforcedDir)
    isWithinRangeList.append(isWithinRange)
...
def integrateTrajAll(x_pred, dXdT_pred, avgXdata):
  ...
  # minX = np.nanmin(x_pred, axis=0)
  # maxX = np.nanmax(x_pred, axis=0)
  ...
  for b in range(nrBiomk):
    ...
    minXcurr = np.nanmin(avgXdata[b])
    maxXcurr = np.nanmax(avgXdata[b])

    try:
      indicesNZ = largestNonZeroCrossing(x_pred[:, b], dXdT_pred[:, b], (minXcurr, maxXcurr))
